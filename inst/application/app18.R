...
ui <- navbarPage("REVA Visualization",
                 ...
                 tabPanel("Aggregate Plots",
                          fluidRow(
                            ...
                            column(10,
                                   use_waiter(),
                                   plotlyOutput("agg_chromPlot_plot"),
...
server <- function(input, output) {
  ...
  output$render_ui_2 <- renderUI({
    list(
      ...
      checkboxInput("log_scale_2", "log10(x + 1)", TRUE),
      numericInput('plot_height_2', 'Download plot height (in)', 3,
                   min = 1, max = 48),
      numericInput('plot_width_2', 'Download plot width (in)', 8,
                   min = 1, max = 48)
    )
  })
  ...
  condition1_chromPlot <- reactive({
   ...
   output$agg_chromPlot_plot <- renderPlotly({
     waiter$show()
     Sys.sleep(3)
     ggplotly(agg_chromPlot_plot())
   })
