...
multi_chromPlot <- function(
  ...
  if(log_scale){
    g <- g + geom_point(aes(x = Feature, y = log10(y.data + 1), col = Chr, shape = Condition, size = 1.5))
  } else {
    g <- g + geom_point(aes(x = Feature, y = y.data, col = Chr, shape = Condition, size = 1.5))
  }
  if(!is.null(data2)){
